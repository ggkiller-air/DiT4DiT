...
def _action_inputs(batch=2):
    return (
        torch.randn(batch, 7, 64),
        torch.randn(batch, 4, 6),
        torch.ones(batch, 4, 6),
        torch.randn(batch, 1, 5),
    )
...
def test_notac_keeps_scalar_action_loss():
    ...
    loss = model(vl, actions, mask, state=state)
    assert not hasattr(model, "tactile_encoder")
    assert loss.ndim == 0 and torch.isfinite(loss)


def test_input_mode_backpropagates_without_teacher():
...
def test_dream_mode_returns_all_losses_without_teacher_gradients():
    model = FlowmatchingActionHead(_action_config("dream", dream_state=True, dream_vision=True))
    vl, actions, mask, state = _action_inputs()
    output = model(
        vl,
        actions,
