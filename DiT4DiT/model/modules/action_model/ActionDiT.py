...
from DiT4DiT.model.modules.action_model.tactile_jepa import (
    RAW_DIM,
    REGION_GRIDS,
    VALID_IDX,
    DreamHead,
...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        if self.use_tactile:
            if not bool(self.model.config.interleave_self_attention):
                raise ValueError("tactile fusion requires diffusion_model_cfg.interleave_self_attention=true")
            ...
            self.tactile_encoder = TactileEncoder(
                ...
                hidden_dim=int(config.get("tactile_hidden_dim", 512)),
        ...
        if self.use_tactile_dream:
            ...
            if self.dream_vision:
                self.vision_target_dim = int(config.get("vision_target_dim", 16))
    ...
    def forward(
        ...
        action_loss = ((pred_actions - velocity) ** 2) * action_mask
        action_loss = action_loss.sum() / action_mask.sum().clamp_min(1)
        ...
        if tactile.ndim != 3 or tactile.shape[1:] != expected_tactile_shape:
            raise ValueError(
                ...
                f"[B, {expected_tactile_shape[0]}, {self.tactile_encoder.raw_dim}], got {tactile.shape}"
            )
